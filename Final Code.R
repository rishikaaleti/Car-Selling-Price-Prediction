...
#ggplot(data = car_df , aes(x=name, fill = name)) + geom_bar() + labs(x='Car Brand') + labs(title = "Bar Graph of Car #Brand") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
#remove blanks and unwanted values
...
#Feature selection comparing methods: exhaustive.
...
#Improving the accuracy of the model by adding log function to numeric variables.
model1 <- lm(log(selling_price) ~log(year)+ log(km_driven)+ seller_type+ transmission+ log(mileage)+ log(max_power) , data = train)
